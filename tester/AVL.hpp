...
namespace ft
{
    ...
    public:
        typedef T value_type;
        typedef AvlNode<const T> const_node;
        typedef Allocator pair_alloc;
        typedef typename Allocator::template rebind<AvlNode<T> >::other node_alloc;

        typedef ft::mapIterator<T, AvlNode<T>, Compare, AVL_tree> iterator;
    ...
    public:
        ...
        AvlNode<T> *_deleteTree(AvlNode<T> *node)
        {
            if (node)
            {
        ...
        iterator begin()
        {
            AvlNode<T> *first = find(FindMin(_root).first);
        ...
        // create newNode
        AvlNode<T> *GetNewNode(T obj)
        {
            AvlNode<T> *newNode = n_alloc.allocate(1);
            n_alloc.construct(newNode);
            newNode->data = p_alloc.allocate(1);
            p_alloc.construct(newNode->data, obj);
            return newNode;
        }
        ...
        AvlNode<T> *find(key val)
        {
            if (doublecheck(_root, val))
                return (find(_root, val));
            return nullptr;
        }
        AvlNode<T> *find(key val) const
        {
            if (doublecheck(_root, val))
                return (find(_root, val));
            return nullptr;
        ...
        AvlNode<T> *getRoot(void) const { return (_root); }
    ...
    private:
        ...
        // contains overload
        bool doublecheck(AvlNode<T> *node, key k) const
        {
            if (node == nullptr)
                return false;
        ...
        int rootGetBalanceFactor(AvlNode<T> *node)
        {
            if (node == nullptr)
            {
                return 0;
        ...
        void ResetParent(AvlNode<T> *oldRoot, AvlNode<T> *newRoot) const
        {
            if (!oldRoot->parent)
            {
                newRoot->parent = NULL;
        ...
        AvlNode<T> *LL_rotation(AvlNode<T> *node)
        { // right rotation
        ...
        AvlNode<T> *RR_rotation(AvlNode<T> *node)
        {
            AvlNode<T> *curr = node->right;
        ...
        /*ft::AvlNode<T> *delete (ft::AvlNode<T> *node, key val)
        {
        }*/

        // overload find
        AvlNode<T> *find(AvlNode<T> *node, key val)
        {
            if (node == nullptr)
                return nullptr;
        ...
        AvlNode<T> *find(AvlNode<T> *node, key val) const
        {
            if (node == nullptr)
                return nullptr;
            bool cmp = _comp(node->data->first, val);
        ...
        void lower_bound(AvlNode<T> *node, key val, AvlNode<T> **pptr) const
        {
            if (node == nullptr)
                return;
            bool cmp = _comp(node->data->first, val);
        ...
        void upper_bound(AvlNode<T> *node, key val, AvlNode<T> **pptr) const
        {
            if (node == nullptr)
                return;
            bool cmp = _comp(node->data->first, val);
        ...
        T findMin(AvlNode<T> *node) const
        {
            if (node == nullptr)
                return (ft::make_pair(key(), value()));
            while (node->left != nullptr)
        ...
        T findMax(AvlNode<T> *node) const
        {
            if (node == nullptr)
                return (ft::make_pair(key(), value()));
            while (node->right != nullptr)
